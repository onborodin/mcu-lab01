...
#define mcu_mmio_set_bite(reg, bite)    (reg) |= (1 << (bite))
#define mcu_mmio_unset_bite(reg, bite)  (reg) &= ~(1 << (bite))

void spi_enable(void) {
    //SPCR |= (1 << SPE);
    mcu_mmio_set_bite(SPCR, SPE);
}

void spi_disable(void) {
    //SPCR &= ~(1 << SPE);
    mcu_mmio_unset_bite(SPCR, SPE);
}

void spi_init(void) {

    //DDRB &= ~(1 << PIN_MISO);   /* Set PIN_MISO as input */
    //DDRB |= (1 << PIN_SS) | (1 << PIN_MOSI) | (1 << PIN_SCK);   /* Set PIN_SS/3/4 as output */
    mcu_mmio_unset_bite(DDRB, PIN_MISO);
    mcu_mmio_set_bite(DDRB, PIN_SS);
    mcu_mmio_set_bite(DDRB, PIN_MOSI);
    mcu_mmio_set_bite(DDRB, PIN_SCS);

    //DDRB |= (1 << PIN_RESET);
    //DDRB |= (1 << PIN_A0);
    mcu_mmio_set_bite(DDRB, PIN_RESET);
    mcu_mmio_set_bite(DDRB, PIN_A0);

    //PORTB &= ~(1 << PIN_SS);    /* Down SS */
    mcu_mmio_set_bite(PORTB, PIN_SS);

    SPCR = (1 << MSTR);         /* Set master mode */

    SPCR &= ~(1 << SPR0);
    SPCR &= ~(1 << SPR1);       /* Speed 1/4 */

    SPSR |= (1 << SPI2X);       /* Set duble speed */

    SPCR &= ~(1 << DORD);       /* Set order */

    SPCR &= ~(1 << CPOL);       /* Set mode */
    SPCR &= ~(1 << CPHA);
...
void lcd_write_command(uint8_t c) {
    mcu_mmio_unset_bite(PORTB, PIN_A0);
    mcu_mmio_unset_bite(PORTB, PIN_SS);
    //PORTB &= ~(1 << PIN_A0);
    //PORTB &= ~(1 << PIN_SS);
    spi_write_byte(c);
    //PORTB |= (1 << PIN_A0);
    mcu_mmio_set_bite(PORTB, PIN_A0);
}
...
void lcd_reset(void) {
    //PORTB |= (1 << PIN_RESET);
    mcu_mmio_set_bite(PORTB, PIN_RESET);

    _delay_ms(500);
    //PORTB &= ~(1 << PIN_RESET);
    mcu_mmio_unset_bite(PORTB, PIN_RESET);
